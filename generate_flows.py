...
def flow(id, priority, eth_type, out_port, in_port='', ip_src='', ip_dst=''):
    flow = {
        "flow": [
            {
                "table_id": 0,
...
def test_second_experiment():
    # Protocolos ARP
    for i in range(1, 4):
        load_flow(flow(id='ARP', priority=4, eth_type=2054, out_port='NORMAL'), i+1)

    load_flow(flow(id=11, priority=1000, eth_type=2048, in_port='1', out_port=2), 1)
    load_flow(flow(id=12, priority=1000, eth_type=2048, in_port='2', out_port=1), 1)
    load_flow(flow(id=21, priority=1000, eth_type=2048, in_port='1', out_port=2), 2)
    load_flow(flow(id=22, priority=1000, eth_type=2048, in_port='2', out_port=1), 2)
    load_flow(flow(id=31, priority=8, eth_type=2048, ip_src='10.0.0.1/32', out_port=1), 3)
    load_flow(flow(id=32, priority=8, eth_type=2048, ip_src='10.0.0.2/32', out_port=2), 3)
    load_flow(flow(id=33, priority=9, eth_type=2048, ip_src='10.0.0.1/32', out_port=3), 3)
    load_flow(flow(id=34, priority=9, eth_type=2048, ip_src='10.0.0.2/32', out_port=4), 3)
    load_flow(flow(id=41, priority=8, eth_type=2048, ip_src='10.0.0.3/32', out_port=1), 4)
    load_flow(flow(id=42, priority=8, eth_type=2048, ip_src='10.0.0.4/32', out_port=2), 4)
    load_flow(flow(id=43, priority=9, eth_type=2048, ip_src='10.0.0.3/32', out_port=3), 4)
    load_flow(flow(id=44, priority=9, eth_type=2048, ip_src='10.0.0.4/32', out_port=4), 4)


# test_second_experiment()
flow(id=11, priority=1000, eth_type=2048, in_port='1', out_port=2)
