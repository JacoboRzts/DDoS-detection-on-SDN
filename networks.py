...
def tree(n_switch=1, n_host=2):
    ...
    net.addLink(switch_array[0], net.addHost(f'server', ip=f'10.0.0.1/16'), bw=4)

    for i in range(1, n_switch+1):
        switch_array.append(net.addSwitch(f's{i+1}', protocols="OpenFlow13"))
        net.addLink(switch_array[i], switch_array[0], bw=8)
        for j in range(n_host):
            net.addLink(switch_array[i], net.addHost(f'h{i}{j+1}', ip=f'10.0.{i}.{j+1}/16'), bw=8)

    switch_array.append(net.addSwitch(name=f's{n_switch+2}', protocols="OpenFlow13"))
    net.addLink(switch_array[-1], switch_array[0], bw=8)
    net.addLink(switch_array[-1], net.addHost('monitor', ip=f'10.0.{n_switch+1}.1/16'), bw=8)
